...
module.exports = function (userSettings) {
  ...
  return function (req, res, next) {
    ...
    if (req.method == "POST" && isObject(req.body) && (req.body.lti_message_type == "basic-lti-launch-request" || req.body.lti_message_type == "ContentItemSelectionRequest")) {
      return options.credentials.call(req, req.body.oauth_consumer_key, function (err, key, secret) {
        ...
        req.lti.valid_request(req, function (err) {
          if (err) {
            console.log("Error in LTI authentication")
            return next(err);
          }
